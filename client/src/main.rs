...
fn main() {
    ...
    match &args.command {
        ...
        Commands::StoreMessage { message_data } => {
            ...
            let mut request_data_map = std::collections::HashMap::new();
            request_data_map.insert("jsonrpc", "2.0");
            request_data_map.insert("id", "1");
            request_data_map.insert("method", "sendtransaction");
            // This should be an array
            request_data_map.insert("params", &encoded_data);

            let client = reqwest::blocking::Client::new();
            let pre_send = client
                .post("http://144.126.212.176:3032/")
                .json(&request_data_map);

            let res = pre_send.send().unwrap();
            println!("{:?}", res.text());
        }
    }
}
